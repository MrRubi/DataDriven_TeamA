...
Aquí podrás encontrar el [GitHub][1] del Equipo A 

[1]: https://github.com/MrRubi/DataDriven_TeamA 

Aquí podrás encontrar el GitHub personal de cada integrante del Equipo A: 
- [Ángel Rubiño Fernández - Mr. T][2] 
- [Álex Gonzalo Rodríguez - Hannibal][3] 
- [José Raúl Jiménez Lama - Fénix][4] 
- [Marc Pallejà Mairena - Murdok (Cuenta 1)][5] 
- [Marc Pallejà Mairena - Murdok (Cuenta 2)][6]
...
```{r tcp_puertos_honeypot, echo= TRUE}
...
```{r udp_puertos_honeypot, echo= TRUE}
#Subset that correlates the host with the TCP protocol and the ports used
...
Analizando los gráficos de los puertos UDP, podemos observar que no se repiten los mismos resultados que en TCP, es decir, que los ataques no se producen en mayor medida en algunos puertos en todos lo honeypots, en este caso están mucho más repartidos entre los diferentes puertos.

Podemos observar, que prácticamente, en cada honeypot destaca un puerto más atacado diferente al resto, y que no destaca por la repetición de dichos puertos, pero si que hay ataques sobre diferentes puertos que aunque no sean los más atacados, se repiten en la mayoría de los honeypots como bien pueden ser el puerto 53 correspondiente a DNS, el 19 correspondiente a Chargen (generador de carácteres), el puerto 5060 correspondiente a SIP (VoIP) o el puerto 1434 correspondiente a Microsoft-SQL-Monitor.
