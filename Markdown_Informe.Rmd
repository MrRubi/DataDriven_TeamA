...
Los autores de la página de donde hemos obtenido los datos son Jay Jacobs y Bob Rudis, profesionales en el campo de security data science, con númerosos años de experiencia contrastada. Jay Jacobs es Chief Data Scientist, fundador y socio en Cyentia Institute, mientras que Bob Rudis es Chief Security Data Scientist en Rapid7. Ambos colaboran en la elaboración del blog (http://datadrivensecurity.info), ademas de ofrecer distintos libros y podcasts sobre Data Driven Security.
...
attacksLocation.graphic <- attacksLocation.graphic + labs(y = "Ataques", x = "Honeypots", title = "Top 3 de paises atacantes por cada honeypot")
...
protocol.graphic <- protocol.graphic + labs(y = "Veces utilizado", x = "Protocolo", title = "Protocolo de trasnporte mas usado")
...
pie(protocol.aggregate$n,labels = lbls,col=rainbow(length(lbls)),
    main="Grafica circular representando el uso de protocols de comunicación") 
```
...
portByProtocol.graphic <- portByProtocol.graphic + labs(y = "Veces usado", x = "Puertos", title = "Top 3 de puertos mas atacados por protocolo")
...
host_tcp_port_gg <- host_tcp_port_gg + labs(y = "Veces usado", x = "Puertos", title = "Puertos mas usados en TCP por honeypot")
...
host_udp_port_gg <- host_udp_port_gg  + labs(y = "Veces usado", x = "Puertos", title = "Puertos mas usados en UDP por honeypot")
